...
export function AppShell({children}:{children:React.ReactNode}){
 ...
 return <div className="min-h-[calc(100vh-33px)] bg-muted/30 lg:grid lg:grid-cols-[240px_1fr]">
   <aside className="hidden border-e bg-background p-4 lg:flex lg:flex-col"><Link href={`/${locale}/calendar`} className="mb-8 flex items-center gap-2 px-2 text-lg font-semibold"><span className="grid size-8 place-items-center rounded-lg bg-primary text-primary-foreground"><Sparkles className="size-4"/></span>TrackMe</Link><nav className="space-y-1">{links.map(({href,label,icon:Icon})=><Link key={href} href={href as never} aria-current={path.startsWith(href)?"page":undefined} className={cn("flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium",path.startsWith(href)?"bg-accent text-accent-foreground":"text-muted-foreground hover:bg-accent/60 hover:text-foreground")}><Icon className="size-4"/>{label}</Link>)}</nav><div className="mt-auto space-y-1"><ThemeToggle className="w-full justify-start text-muted-foreground"/><Button onClick={logout} variant="ghost" className="w-full justify-start text-muted-foreground"><LogOut className="size-4"/>{t("logout")}</Button></div></aside>
   <main className="min-w-0 pb-20 lg:pb-0">{children}</main>
   <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t bg-background/95 px-2 py-2 backdrop-blur lg:hidden">{links.map(({href,label,icon:Icon})=><Link key={href} href={href as never} aria-current={path.startsWith(href)?"page":undefined} className={cn("flex min-w-0 flex-1 flex-col items-center gap-1 rounded-md px-1 py-1 text-[11px]",path.startsWith(href)?"text-foreground":"text-muted-foreground")}><Icon className="size-5"/><span>{label}</span></Link>)}<ThemeToggle className="h-auto min-w-0 flex-1 flex-col gap-1 px-1 py-1 text-[11px] font-normal text-muted-foreground"/></nav>
 </div>
}
