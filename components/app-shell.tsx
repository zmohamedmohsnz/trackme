...
export function AppShell({children}:{children:React.ReactNode}){
 ...
 return <div className="min-h-[calc(100vh-33px)] bg-muted/30 lg:grid lg:grid-cols-[240px_1fr]">
   ...
   <nav className="fixed inset-x-0 bottom-0 z-40 flex justify-around border-t bg-background/95 px-2 py-2 backdrop-blur lg:hidden">{links.map(({href,label,icon:Icon})=><Link key={href} href={href as never} className={cn("flex min-w-20 flex-col items-center gap-1 rounded-md px-2 py-1 text-[11px]",path.startsWith(href)?"text-foreground":"text-muted-foreground")}><Icon className="size-5"/><span>{label}</span></Link>)}</nav>
 </div>
}
